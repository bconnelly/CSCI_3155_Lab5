object Lab5 extends jsy.util.JsyApplication {
  ...
  //return the list when we get to the end of the list
  case Nil => doreturn(l)
  //If the list isn't empty...
  case h :: t => f(h) match {
    //if the function returns none, recurse on the tail and map the list to a modified
    //version with the head attached to a modified tail
    case None => mapFirstWith(f)(t).map((alpha:List[A]) => (h::alpha))
    //if it returns Some, map the new modified alpha to the tail
  ...
  def castOk(t1: Typ, t2: Typ): Boolean = (t1, t2) match {
    ...
    //iterate through all the fields in the first object
    case (TObj(fields1), TObj(fields2)) => fields1.forall{
      //short circuit - return true if type is None
      case(f1,typ1) if (typ1 == None) => true
      //otherwise, match on the field of second object that matches field in first object
      case(f1,typ1) => fields2.get(f1) match { 
        //base case - return true if no such field
        case None => true
        //recurse with type of field in first object and the type of the 
  ...
  def typeInfer(env: Map[String,(Mutability,Typ)], e: Expr): Typ = {
    ...
    e match {
      ...
      //TypeDecl
      //Extend the environment with x mapped to its mode and type, then infer e2 
      ...
      //p is function name 
      //paramse is of type Params =which is Either[ List[(String,Typ)], (PMode,String,Typ) ]
      //first is like lab4, second is Passingmode(ref,val,etc)
      //tann is return type
      //e1 is evaluated expression 
      
      case Function(p, paramse, tann, e1) => {
        //if the function is named (can be recursive), add it to the env
        ...
        //env2 is env1 further extended with the parameters 
        val env2 = paramse match {
          ...
          //if the arg is being passed by name, it's const
          //in any other case it's var
          case Right((mode,x,t)) => mode match{
            case PName => env1 + (x -> (MConst,t))
            case _ => env1 + (x -> (MVar,t))
          }
        }
        //Now we can infet the type of the function body
	 ...
	 case Assign(e1, e2) => e1 match{
	   //e1 is either Var(x) or GetField
	   
	   case Var(x) => env.get(x) match {
	     //throw error if trying to reassign constant
	     case Some((MConst, t)) => err(typ(e1), e2)
	     //if types match, extend env with x (val of e1) and infer type of e2. Otherwise throw error.
	     case Some((MVar, t)) => if (t == typ(e2)) {
	         typeInfer(env + (x -> (MVar, typ(e2))), e2)
	       } else {
	         err(typ(e1), e2)
	       }
	     //if var is unnamed, extend env with x (val of e1) and infer type of e2
	     case _ => typeInfer(env + (x -> (MVar, typ(e2))), e2)
	   }
	   //if e1 is GetField, extend env with the field and infer type of e2
	   case GetField(x1, f) => typ(e1) match {
	     case t => typeInfer(env + (f -> (MConst, typ(e1))), e2)
	   }
	   case _ => err(typ(e1), e2)
	}
	  //Just use castOk if casting
  ...
  def substitute(e: Expr, esub: Expr, x: String): Expr = {
    ...
    ep match {
      ...
      //tann is return type
      //e1 is evaluated expression 
      case Function(p, Left(paramse), rettyp, e1) => p match{
        //if function is named...
    	case Some(a) =>
    	  	//if the parameter names and function name don't match x, resursively substitute in e1
    	  	//otherwise e2 = e1
      		val e2 = if (paramse.forall{ case(n,_) => (x!= n) } && a!=x) subst(e1) else e1
      		//return the function with newly substituted body
      		Function(p, Left(paramse), rettyp, e2)
     	 //if function is unnamed...
    	case None =>
      		//same, but don't check function name against x
  	...
  	case Function(p, Right(paramse@(mode,s,t)), retty, e1) => p match{
  		//if function is named...
    	case Some(a) =>
    	  	 //sub on e1 if function arg and name don't match x
    	  	 //return either substituted function or original function if sub is not necessary
     		 Function(p, Right(paramse), retty, if (x != s && x != a) subst(e1) else e1)
    	case None =>
     		 //same, but don't check function name against x
     		 Function(p, Right(paramse), retty, if (x != s) subst(e1) else e1)
  	}
  ...
  def step(e: Expr): DoWith[Mem, Expr] = {
	...
	e match {
 	...
 	//if fields are all values, add object to memory
  	case Obj(fields) if (fields forall { case (_, vi) => isValue(vi)}) =>
    	Mem.alloc(Obj(fields)) map {(a:A) => a:Expr}
    //
  	case GetField(a@A(_), s) => {
    ...
    //just step on e1 unless it's Null, in which case we throw a NullDereferenceError
  	case GetField(e1, f) =>
    ...
    //straightforward stepping on e1 in each case, in Assign case step on e2 after e1
